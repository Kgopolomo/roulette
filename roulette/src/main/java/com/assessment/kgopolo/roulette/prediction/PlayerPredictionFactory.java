...
import com.assessment.kgopolo.roulette.prediction.Prediction.Type;
...
class PlayerPredictionFactory {
	...
	public Optional<PlayerPrediction> createPlayerBet(String input) {
		return Optional.ofNullable(input).flatMap(val -> {
			...
			if (playerBetMatcher.matches()) {
				...
				return Optional.of(name).flatMap(this::findPlayer).map(player -> new PlayerPrediction(player, crateBet(playerBetMatcher)));
			} else {
	...
	private Prediction crateBet(Matcher playerBetMatcher) {
		...
		return new Prediction(betNumber, betAmount, betType);
	}
